...
def gerar_pdf(dados, caminho):
    ...
    dados_basicos = [
        ...
        [
            Paragraph("<b>Pontos de Vida</b><br/>" + str(dados["Pontos de Vida"]), texto),
            Paragraph("<b>Dados de Vida</b><br/>" + str(dados["Dados de Vida"]), texto),
            
        ],
        [
            Paragraph("<b>Classe de Armadura</b><br/>" + str(dados["Classe de Armadura"]), texto),
            Paragraph("", texto)
        ],

    ]

    tabela = Table(dados_basicos, colWidths=[92 * mm, 92 * mm], rowHeights=[14 * mm] * 5)
    ...
    for atributo in ATRIBUTOS:
        ...
        atributos_linha.append(
            Paragraph(f"<b>{atributo.upper()}</b><br/><br/>{valor}", texto)
        )
    ...
    campos = [
        ("Deslocamento", dados["deslocamento"], 18),
        ("Visão", dados["visao"], 18),
        ("Vantagem", dados["vantagem"], 18),
        ("Idiomas", dados["idiomas"], 18),
        ("Proficiência em combate", dados["proficiencia_combate"], 45),
        ("Proficiência em ferramentas", dados["proficiencia_ferramenta"], 45),
        ("Especialização", dados["especializacao"], 45),
        ("Equipamentos e Inventário", "", 50),
        ("Magias / Habilidades", "", 60),
        ("Características e Anotações", "", 65),
    ]

    for nome, dado, altura in campos:
        bloco = Table(
            [[Paragraph(f"<b>{nome}</b>", texto)], [Paragraph(f"{dado}", texto)]],
            colWidths=[184 * mm],
...
def obter_dados():
    ...
    return {
        ...
        "vantagem": p.vantagem,
        "visao": p.visao,
        "proficiencia_combate": p.proficiencia_combate,
        "proficiencia_ferramenta": p.proficiencia_ferramenta,
